...
void _validateUsername(String username) {
  setState(() {
    if (username.isEmpty) {
      ...
      if (RegExp(r'\d').hasMatch(username) || !RegExp(r'^[a-zA-Z0-9]+$').hasMatch(username)) {
        emailError = 'Username must be at least 3 characters long and cannot start with digits.';
      } else {
    ...
    }  else if (username[0] != username[0].toUpperCase()) { // Check if first character is capital
  ...
  Future<void> _login() async {
    ...
    if (emailError.isNotEmpty || passwordError.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Please choose correct username and password for logging.")),
      );
      return;
    }
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: <Widget>[
                      ...
                      Container(
                        decoration: BoxDecoration(
                          color: emailError.isNotEmpty ? Colors.red[100] : Colors.white,
                      ...
                      Container(
                        decoration: BoxDecoration(
                          color: passwordError.isNotEmpty ? Colors.red[100] : Colors.white,
                          borderRadius: BorderRadius.circular(10),
                        ),
                        child: TextField(
...
class _ResetPasswordPageState extends State<ResetPasswordPage> {
  ...
  void _validateEmail(String email) {
    setState(() {
      if (email.isEmpty) {
        ...
        if (RegExp(r'\d').hasMatch(email) || !RegExp(r'^[a-zA-Z0-9]+$').hasMatch(email) || email[0] != email[0].toUpperCase()) {
          emailError = 'Username must be at least 3 characters long and cannot contain digits or special characters with upper case first letter.';
        } else {
          emailError = 'Username must be at least 3 characters long.';
        }
  ...
  Future<void> _resetPassword() async {
    ...
    if (emailError.isNotEmpty || passwordError.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Please fix the errors before resetting password.")),
      );
      return;
    }
    ...
    if (response.statusCode == 200) {
      ...
      if (errorMessage.contains('Password must be unique')) {
        setState(() {
          passwordError = 'The password must be unique. Please choose a different password.';
          isPasswordUnique = false; // Set to false if the password is not unique
        });
      }
              ...
              decoration: InputDecoration(
                ...
                border: OutlineInputBorder(
                  borderSide: BorderSide(
                    color: isPasswordUnique ? Colors.grey : Colors.red, // Set red border if password is not unique
                    width: 2,
                  ),
                ),
                errorText: passwordError.isNotEmpty ? passwordError : null,
